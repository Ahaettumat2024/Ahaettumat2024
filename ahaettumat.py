...
col313.metric('Escapes per 1000 tons', round(( st.session_state['farmNumbersEarly'].to_numpy().sum()+ st.session_state['farmNumbersLate'].to_numpy().sum())/(ITERS*1000*st.session_state['eldi'].loc[:,'Stock'].sum()),1))
